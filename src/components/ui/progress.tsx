...
const Progress = <T extends ValidComponent = "div">(props: PolymorphicProps<T, ProgressRootProps<T>>) => {
    const [local, others] = splitProps(props as ProgressRootProps, ["children", "fill"])
    return (
        <ProgressPrimitive.Root {...others}>
            ...
            <ProgressPrimitive.Track class="relative h-2 w-full overflow-hidden rounded-full bg-secondary">
                <ProgressPrimitive.Fill
                    class={"h-full w-[var(--kb-progress-fill-width)] flex-1 transition-all ".concat(
                        local.fill ?? "bg-primary",
                    )}
                />
